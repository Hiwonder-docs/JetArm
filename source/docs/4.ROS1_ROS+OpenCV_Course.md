...
The parameter **self.D** corresponds to the camera's intrinsic parameters$\begin{bmatrix}
fx & 0 & cx \\
0 & fy & cy \\
0 & 0 & 1
\end{bmatrix}$.
...
(12) Invoke `set_servos()` function to control the robot arm.
...
<img class="common_img" src="../_static/media/chapter_4/section_1/media/image253.png"  />

Taking the function `set_servos(self.servos_pub, 500, ((1, 500), ))` as an example:

The first parameter, `self.servos_pub`, represents the topic message.

The second parameter, `500`, denotes the servo's runtime.

The third parameter, `(1, 500)`, consists of the servo ID (1) and the servo rotation position (500).

(13) If the robot arm finishes the gripping action, it will switch to the putting down mode.
...
<img class="common_img" src="../_static/media/chapter_4/section_1/media/image254.png"  />

The parameters `position.x`, `position.y`,  and  `position.z` represent the coordinates for placing the color block. If the placement is inaccurate, the parameters for x and y can be adjusted.

(14) If the robot arm has placed the color block to the designated place, the robot arm will programmed to restore the initial pose.
...
Call the `get_endpoint()` function to acquire the endpoint position of the robot arm.

(15) If the game is canceled or the robotic arm cannot reach the gripping or placing position, instruct the robotic arm to stop operation and return to the initial pose.
...
Invoke the `done_callback()` function to check if gripping is completed; if so, proceed with placing, otherwise return to the initial position.

Call the `active_callback()` for the start of motion

Use the `feedback_callback()` for the progress callback during action execution.

- **image_callback() Function**

Process the image to obtain the coordinate of the target to be picked up.
...
(1) Convert the format of the image.
...
(3) Create a mask for the recognition area.
...
(4) Preserve the region to be recognized.
...
(5) Convert the color space into LAB space.
...
(6) Perform binary processing and apply smoothing filtering to the image.
...
(7) Find out all contours, and calculate the contour area.
...
(8) Draw the minimum bounding rectangle around the maximum area.
...
(9) Obtain the angle of the rectangle to facilitate adjusting the gripper's angle during subsequent gripping.
...
(10) When moving_step is equal to 0, it is the color recognition stage. At this point, color processing is applied.
...
(11) When the detected target color exceeds 50 times, transform the coordinate system of the robotic arm.
...
(12) Set the status of the robot arm to 1, which enables the robot arm to start gripping.
...
(13) The terminal displays the live camera feed.
