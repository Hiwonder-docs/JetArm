...
(1) Extract the virtual machine software package saved in **[2. Software Tool/ 6. Voice Environment Setup Tool/ Virtual Machine Installation Pack](https://docs.hiwonder.com/projects/JetArm/en/latest/docs/resources_download.html)**.
...
(1) In the popup window, locate the image file under the directory "**[2 Tools/6 Voice Recognition Environment Set Up Tool/6-Channel Microphone Array Image](https://docs.hiwonder.com/projects/JetArm/en/latest/docs/resources_download.html)**". Open the image file (the actual name of the image may vary; this image is for reference only).
...
[9.7.2 Replace Offline Voice Resources and ID](https://docs.hiwonder.com/projects/JetArm/en/latest/docs/9.ROS1_Voice_control.html#replace-offline-voice-resources-and-id)
...
(2) Please calibrate the position of the robot arm before starting the voice-controlled color sorting function. To get detailed instruction, please refer to [1.Getting Ready(JetArm User Manual)-> 1.4.5 Position Calibration]().
...
* In case of an incorrect recognition result, follow the instructions provided in [1. Getting Ready( JetArm User Manual)->1.5 Color Threshold Setting]() to adjust the color threshold for a more accurate outcome.
...
If you wish to use a wake word in a different language, please refer to the guide titled ["17.2 Switching Between Chinese and English Wake Words"](#anchor_18_2) 
...
For learning about the PID algorithm, refer to "[12. ROS2- OpenCV Vision Course->12.2 Target Tracking](https://docs.hiwonder.com/projects/JetArm/en/latest/docs/4.ROS1_ROS%2BOpenCV_Course.html#target-tracking)"
...
* If there is an incorrect recognition result, refer to the instructions in [1. Getting Ready(JetArm User Manual)->1.5 Color Threshold Setting]() to adjust the color threshold for a more precise outcome.

:::

(1) Start the robot, and access the remote desktop using NoMachine. For detailed instruction, please refer to the file saved in [1. Getting Ready(JetArm User Manual)->1.5 Color Threshold Setting]().
